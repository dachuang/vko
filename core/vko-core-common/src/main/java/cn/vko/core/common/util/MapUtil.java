...
public class MapUtil {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <K1, K2, V> List<V> getMapMap(final Map<K1, Map<K2, V>> map, final K1 key) {
		if (isEmpty(map)) {
			return Lang.list();
		}
		if (map.containsKey(key)) {
	 ...
	 * @return map对象
	 */
	public static <K1, K2, V> Map<K1, Map<K2, V>> addMapMap(final Map<K1, Map<K2, V>> map, final K1 key, final K2 key2,
			final V value) {
		checkNull(map, "填充map时,数据为空!");
		if (map.containsKey(key)) {
	 ...
	 * @return value
	 */
	public static <K1, K2, V> V getMapMap(final Map<K1, Map<K2, V>> map, final K1 key, final K2 key2) {
		if (isEmpty(map)) {
			return null;
		}
		if (map.containsKey(key)) {
	 ...
	 * @param key2 里层map的键值
	 */
	public static <K1, K2, V> void removeMapMap(final Map<K1, Map<K2, V>> map, final K1 key, final K2 key2) {
		if (isEmpty(map)) {
			return;
		}
		if (map.containsKey(key)) {
