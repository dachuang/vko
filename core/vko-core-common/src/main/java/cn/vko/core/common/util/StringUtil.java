...
public class StringUtil {
	...
	*/
	...
	public static boolean isNumber2(final String str) {
		if (isEmpty(str)) {
			return false;
		}
		String astr = str;
	 ...
	 * @param str 被替换字符串
	 * @param re 需要替换的内容
	 * @param replaceChar 替换成的char
	 * @return 替换后的字符串
	 */
	public static String replace(final String str, final List<String> re, final char replaceChar) {
		if (isEmpty(str)) {
			return "";
		}
		if (isEmpty(re)) {
		...
		for (String reStr : re) {
			...
			result = result.replaceAll(reStr, Strings.alignLeft("", reStr.length(), replaceChar));
		}
		return result;
	}
