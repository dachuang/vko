...
public class RedisQueueUtil {
	 ...
	 * @return 队列中元素个数
	 */
	public static long push(final IRedisDao dao, final String type, final Map<String, String> values) {
		if (isEmpty(values)) {
			return 0;
		}
		checkDao(dao);
		...
		dao.lpush(key(QUEUE, type), CollectionUtil.collection2array(idList, String.class));
	 ...
	 * @return 队列中元素个数
	 */
	public static long push(final IRedisDao dao, final String type, final String key, final String value) {
	 ...
	 * @return 队尾对应的主键
	 */
	public static void back2Error(final IRedisDao dao, final String type, final String id) {
		checkDao(dao);
		checkEmpty(type, "类型不能为空");
	 ...
	 * @param id 主键
	 */
	public static void finish(final IRedisDao dao, final String type, final String id) {
		checkDao(dao);
		checkEmpty(type, "类型不能为空");
	 ...
	 * @param entity 待重做的任务
	 */
	public static void redo(final IRedisDao dao, final String type, final String id) {
		checkDao(dao);
		checkEmpty(type, "类型不能为空");
		checkEmpty(id, "主键不能为空");
