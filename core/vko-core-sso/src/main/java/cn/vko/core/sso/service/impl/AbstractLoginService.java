...
public abstract class AbstractLoginService implements ILoginService {
	...
	@Override
	public String login(final String loginName, final String password, final boolean rememberme) {
		SsoUser u = getUser(loginName);
		if (u == null) {
			throw bEx(ErrorType.NOTEXIST.value());
		}
		u.checkPass(password);
		if (!u.isValid()) {
			throw bEx(ErrorType.INVALID.value());
		}
