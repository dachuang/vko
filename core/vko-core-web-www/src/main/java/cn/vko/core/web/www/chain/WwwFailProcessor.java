...
public class WwwFailProcessor extends FailProcessor {
	...
	*/
	private static final Logger logger = LoggerFactory.getLogger(WwwFailProcessor.class);

	private KvConfig config;

	@Override
	protected void handleError(final ActionContext ac, final Throwable th) {
		if (th instanceof IBusinessException || th instanceof ITimeoutException || th instanceof IParamException) {
			return;
		}
		String url = "unknown";
