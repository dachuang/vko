...
public class FtlUtil {
	...
	public static void initConfiguration(final Configuration cfg, final ServletContext context,
			final String relativePath, final String propPath) {
		...
		try {
			...
			String f = Thread.currentThread().getContextClassLoader().getResource("").getPath() + propPath;
	 ...
	 * @return 返回的字符串
	 */
	public static String build(final Configuration cfg, final String templateName, final Map<String, Object> context) {
		try {
			@Cleanup
